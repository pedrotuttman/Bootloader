...
mov ax, 0x7c0
mov es, ax ; es eh oregistrador de seguiment0o utilizado na interrupçao 

sub ax, 0x200 ; Estabelecemos uma regiao para a stack que será utilizada mais a frente
mov sp, ax
mov bp, ax
...
; INT 13h AH=02h : Ler setores da unidade
; Parâmetros
; AH	02h
; AL	Contagem de setores a serem lidos
; CH	Cilindro
; CL	Setor
; DH	Cabeça
; DL	Dirigir
; ES:BX	Ponteiro de endereço do buffer

mov ah, 2
mov al, 3 ; numero de setores para ler
mov ch, 0 ; numero do cilindro
mov cl, 2 ; porque o primeiro ja está carregado
...
push 0x0f ; empurrando o branco para a pilha
push 0x00 ; empurrando o valor de x
push 0xc7 ; empurra o valor de y (comeca de baixo, ent y = 199 em decimal)
push 0x00 ; empurra o numero de repeticoes que faremos 

; estamos lidando com 16 bits (2bytes)
; entao o valor das repeticoes esta no sp, de y esta em sp + 2, o de x em sp + 4 e a cor em sp + 6 

;assembly-16bits nao deixa realizarmos algumas operacoes com o sp, logo, movemos para o di
...
volta1: 
xor bh, bh
mov cx, [di + 0x04] ; horizontal - X
mov al, [di + 0x06] ; cor
mov ah, 0x0c
int 0x10
dec bl ; diminui 1 repeticao
mov [di], bl ; salvo as repeticoes que faltam
...
mov ax, word [di + 0x04]
inc ax
cmp ax, 0x13f ; compara com 319, ja que nosso desenho eh 320x200
je preparaRetorno
mov [di + 0x04], ax
jmp volta1
...
preparaRetorno:
xor ax, ax
mov [di + 0x04], ax ; X - horinzontal
mov ax, [di + 0x02] ; Y - vertical
dec ax
mov [di + 0x02], ax
inc si
mov bl, byte [0x7c00 + dados + si] ; repeticoes
...
trocaCor:
mov al, [di + 0x06]
cmp al, 0x0f ; verifica se é branco
je preto 
mov al, 0x0f
jmp salva
...
salva:
mov [di + 0x06], al
inc si ; pega a proxima repeticao

mov bl, byte [0x7c00 + dados + si] ; repeticoes
cmp bl, 0xff ; compara com 255
je final
mov [di], bl
jmp volta2
...
dados:
    ...
    db 47,127,126,5,20,23,146,162,8,150,166,2,152, 255
